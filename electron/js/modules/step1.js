...
function validateDroppedFile(f) {
  ...
  if (f.webkitRelativePath && f.webkitRelativePath !== "") {
    showFileError(
      "Folders are not supported — please drop a PDF, DOCX, TXT, or XLSX file."
    );
    return false;
  }
  ...
  if (!name.match(/\.(pdf|docx|doc|txt|xlsx|xls)$/)) {
    showFileError(
      "Unsupported file type — only PDF, DOCX, TXT, and XLSX files are accepted."
    );
    return false;
  }
  return true;
...
function onDrop(e) {
  ...
  const dropped = Array.from(e.dataTransfer.files).filter((f) =>
    validateDroppedFile(f)
  );
  if (dropped.length) {
    window.S.files = dropped;
    window.S.file = dropped[0];
    window.S.filePath = null;
    showFiles(dropped);
  }
}
...
async function pickFile() {
  ...
  if (input) {
    ...
    if (!r.canceled && r.filePaths[0]) {
      window.S.filePath = r.filePaths[0];
      window.S.file = null;
      window.S.files = [];
      ...
      .map(
        (f) =>
          `<div><strong>${esc(f.name)}</strong>${
            f.size
              ? `<span style="font-size:11px;margin-left:6px;color:var(--color-text-muted)">${fmt(
                  f.size
                )}</span>`
              : ""
          }</div>`
  ...
  try {
    ...
    if (allFiles.length > 0) {
      ...
      const resp2 = await fetch(
        "file:///" + window.S.filePath.replace(/\\/g, "/")
      ).catch(() => null);
      if (resp2) {
    ...
    const r = await fetch(`http://127.0.0.1:${window.S.port}/parse`, {
      method: "POST",
      body: fd,
    ...
    window.S.sourceType = _srcName.toLowerCase().endsWith(".pdf")
      ...
      : _srcName.toLowerCase().endsWith(".docx") ||
        _srcName.toLowerCase().endsWith(".doc")
      ? "docx"
      : "txt";
    ...
    const bundleItems = Array.isArray(data.data.line_items)
      ? data.data.line_items.filter((i) => i && typeof i === "object")
      : [];
    const aiItems = Array.isArray(data.data.ai_line_items)
      ? data.data.ai_line_items.filter((i) => i && typeof i === "object")
      : [];
    if (bundleItems.length) {
      ...
      window.S.items = data.data.quantities.map((q, idx) => ({
        id: idx + 1,
        description: data.data.project_title || "",
        size: q.size || "",
        unit: "EA",
        qty: q.qty || "",
        unit_price: "",
    ...
    if (e.name === "AbortError") {
      e.message =
        "Parsing timed out. The file may be too large or corrupted — try a smaller document.";
    }
    ...
    if (e.message && e.message.includes("Failed to fetch")) {
      msg =
        "Could not reach the backend server. Make sure the application is fully started and try again.";
    } else if (e.message && e.message.includes("timed out")) {
      msg =
        "Extraction timed out — try a smaller file or check your connection.";
...
(function () {
  ...
  document.addEventListener("dragover", (e) => {
    if (window.S.step === 1) e.preventDefault();
  });
...
function step1(c) {
  const resumeBanner = window.S._pendingSession
    ...
    <div class="alert alert-info" style="margin-bottom:12px;display:flex;align-items:center;gap:10px">
      <div style="flex:1">
        ...
        <div style="font-size:11px;margin-top:2px">${esc(
          window.S._pendingSession.extracted?.solicitation_number || ""
        )} ${esc(window.S._pendingSession.extracted?.project_title || "")}</div>
      </div>
  ...
  <div class="speed-dial">
    ...
    <div class="sd-items" id="sd-items">
      <button class="btn btn-ghost btn-sm sd-item" id="qa-ai-btn">AI Extraction${
        window.S.apiKey
          ? ' <span style="color:var(--color-success);font-size:10px">&#9679; active</span>'
          : ""
      }</button>
  ...
  if (fileInput) {
    fileInput.addEventListener("change", () => {
      ...
      if (selected.length) {
        window.S.files = selected;
        window.S.file = selected[0];
        window.S.filePath = null;
        showFiles(selected);
      }
    });
  ...
  document
    .getElementById("sd-trigger")
    ?.addEventListener("click", toggleQuickActions);

  // Wire quick action items (functions still live in index.html script block)
  document
    .getElementById("qa-ai-btn")
    ?.addEventListener("click", () => window.openAiModal?.());
  document
    .getElementById("qa-sam-btn")
    ?.addEventListener("click", () => window.openSamModal?.());
  document
    .getElementById("qa-demo-btn")
    ?.addEventListener("click", () => window.loadDemoData?.());

  // Wire resume/dismiss session buttons if present
  document
    .getElementById("resume-session-btn")
    ?.addEventListener("click", () => window.resumeSession?.());
  document
    .getElementById("dismiss-session-btn")
    ?.addEventListener("click", () => window.dismissSession?.());
  ...
  if (window.S.files && window.S.files.length > 0) showFiles(window.S.files);
    ...
    showFiles([{ name: window.S.filePath.split(/[/\\]/).pop() }]);
}
